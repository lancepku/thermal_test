...
if (iac_rem.eq.2) then
 ...
 do j = 1,nz
 cord(j,1,1) = cordo(j,2,1) -dx_init
end do
endif
...
enddo
do j= 1,nz
  ! cord(j,iinj-2,1) = cord(j,iinj-3,1) +dx_init
   cord(j,iinj-1,1) = cord(j,iinj-2,1) +dx_init
   cord(j,iinj,1) = cord(j,iinj-1,1) + dx_init 
   !cord(j,iinj+4,1) = cord (j,iinj+5,1)-dx_init
   !cord(j,iinj+3,1)= cord(j,iinj+4,1) - dx_init
   cord(j,iinj+2,1)= cord(j,iinj+3,1) -dx_init
   cord(j,iinj+1,1) = cord(j,iinj+2,1) -  dx_init 
enddo
  return
endif
! remeshing the extrusion layer thickness (Zl Liu ,4/6/2017)
!if (iac_rem.eq.1)then
!  do i = 1,iinj-1
!     dhext(i) = dhext(i+1)
!  enddo
