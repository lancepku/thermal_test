...
boff = 0
! Vary the left side velocity 
!wt = -real ((cord(1,1,2)+cord(1,2,2)+cord(1,3,2))*1.0/3.0*1.0e-3)
!write(*,*)'wt=',wt
!  vel_l0= bca(1)
!do j = 1,nz 
! bc(j,1,1)= vel_l0 + 0.5*wt*0.2*1.6e-9
!end do
!write(*,*)'bc-1',bc(nz/2,1,1)
drat = dt / dt_elastic
if (drat .lt. 1.) drat = 1.
...
do i=1,nx
    do j=1,nz
        ...
        ! Velocities from Wt by Zhonglan
!            vel(j,1,1) = vel (j,1,1) + 0.5*0.2*wt*1.0e-9
        ! MAX balance-off
