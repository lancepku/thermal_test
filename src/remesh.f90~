...
allocatable :: dummy(:,:), cordo(:,:,:), dhnew(:),dhnew_1(:)
...
!#################################
!Interpolate extrusion layer thickness (ZL Liu, 04/07/2017)
allocate (dhnew_1(nx-1))
dhnew_1(:) = 0
  ...
  ! for each old node, starting from i2
   do i1 = i2,nx-1
     ...
     if (cordo(1,i1,1) <  cord(1,i+1,1)) then
        if (i1 /= 1) then
           if (cordo(1,i1-1,1) >= cord(1,i,1)) then
             dhnew_1(i) = dhnew_1(i) + dhext(i1-1) * (cordo(1,i1,1)- cordo(1,i-1,1))
           else
             dhnew_1(i) = dhnew_1(i) + dhext(i1-1) * (cordo(1,i1,1) - cord(1,i,1))
           end if 
      end if
     else
       if (i1 /= 1) then
             dhnew_1(i) = dhnew_1(i) +dhext(i1-1) * (cord(1,i+1,1)-cordo(1,i1-1,1))
       end if
       i2 = i1
       exit
      end if
...
! special treatment for the last new element
  i = nx-1
  i1 = i2
   if (cordo(i,i1,1) <= cord(1,i,1)) then
     dhnew_1(i) = dhnew_1(i) + dhext(i1) * (cord(1,i+1,1)-cord(1,i,1))
   end if
  dhext(1:nx-1) = dhnew_1/(cord(1,2:nx,1)-cord(1,1:nx-1,1))
  dhext(iinj) = 0
deallocate(dhnew_1) 
!##############################
!Linear Interpolate extrusion layer thickness (ZL Liu, 04/09/2017)                                                                                                                                 
!allocate (dhnew_1(nx-1))
!dhnew_1(:) = 0
!i2=1
! for each new element i                                                                                                                                                                   
...
!   for each old node, starting from i2                                                                                                                                                    
!   do i1 = i2,nx-1
!     if (cord(1,i,1) <= cordo(1,i1,1)) then
!       if (i1 /= 1) then
!        dhnew_1(i) = dhext(i1-1) + (dhext(i1)-dhext(i1-1))*(cord(1,i,1)-cordo(1,i1-1,1))/(cordo(1,i1,1)&
!      -cordo(1,i1-1,1))
!       else
!       dhnew_1(i) = dhext(i1)
!       end if
...
!     if (cord(1,i,1) > cordo(1,i1,1)) then
!      if (i1 /= nx-1) then
!        dhnew_1(i) = dhext(i1+1) - (dhext(i1+1)-dhext(i1))*(cordo(1,i1+1,1)-cord(1,i,1))/&
!        (cordo(1,i1+1,1)-cordo(1,i1,1))
!      else
!       dhnew_1(i) = dhext(i1)
!      end if
!     end if
!   end do
!end do
!dhext(1:nx-1) = dhnew_1(1:nx-1)
!special treatment for the dike 
!dhext(iinj) = 0
