...
case (1)
    ...
    do i = 1,nx-1
        do j = 1,nz-1
            ...
            if(igeotherm .eq.3)then
                if ((xc-g_x0).lt.0)then 
                    if ((xc-g_x0).lt.(-g_width)) geoth = g_y0c 
                    if ((xc-g_x0).ge.(-g_width)) geoth = g_y0c + g_amplitude*(1-abs(g_x0-xc)/g_width)
                endif
                if ((xc-g_x0).ge.0) then
                    if ((xc-g_x0).lt.(1.4*g_width)) geoth = g_y0c +g_amplitude*(1-abs(g_x0-xc)/(1.4*g_width))
                    if ((xc-g_x0).ge.(1.4*g_width)) geoth = g_y0c
                endif
              endif
            ! Temperatures
            ...
            if(yc.ge.geoth) then
                ...
                temp(j,i)=tbos + ((tbos-t_top)/(0.05*geoth))*(yc-geoth)
            endif
